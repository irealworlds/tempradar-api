    ...
    (IPinnedSensorService pinnedSensorService) : AuthorizationHandler<OperationAuthorizationRequirement, PinnedSensor>
{
        ...
        PinnedSensor resource)
    {
        ...
        {
            if (await pinnedSensorService.UserCanReadSensorAsync(context.User, resource)) context.Succeed(requirement);
        }
        ...
        else
        {
            context.Fail(new AuthorizationFailureReason(this,
                $"Operation {requirement.Name} not handled for resource of type {nameof(PinnedSensor)}."));
        }
    }
}