...
public class PinnedCityWeatherService(IPinnedCityService pinnedCityService,
    ICurrentWeatherService currentWeatherApiService, IWeatherHistoryService weatherHistoryService,
    IMapper mapper) : IPinnedCityWeatherService
{
    ...
    public async Task<List<CityDailyTemperatureDto>?> GetWeatherHistoryAsync(Guid id)
    {
        ...
        var weatherHistory =
            await weatherHistoryService.GetWeatherHistoryForLocationAsync(city.Latitude, city.Longitude, startDate,
                endDate);

        if (weatherHistory.Forecast?.DailyForecasts == null) return null;
