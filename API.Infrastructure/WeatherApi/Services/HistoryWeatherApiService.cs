...
{
    public async Task<WeatherHistoryDto> GetWeatherHistoryForLocationAsync(double latitude, double longitude,
        DateTime date)
    {
        var requestUri = this.BuildEndpointUri("/history.json", new Dictionary<string, string>
        {
        ...
        if (weatherHistoryDto == null) throw new Exception("Could not parse data returned from WeatherApi.");
